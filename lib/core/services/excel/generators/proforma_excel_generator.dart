...
class ProformaExcelGenerator {
  static Future<Uint8List?> generate(
    ...
    titleCell.value = TextCellValue('PROFORMA INVOICE');
    ...
    sheetObject
        ...
        .value = TextCellValue(
      proforma.proformaNumber.toString(),
    );
    sheetObject
            .cell(
              CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: currentRow),
    ...
    sheetObject
        ...
        .value = TextCellValue(
      proforma.date != null
          ? DateFormat('dd-MMM-yy').format(proforma.date!)
          : '-',
    );
    currentRow++;
    ...
    sheetObject
        .cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: currentRow))
        .value = TextCellValue(
      proforma.client.address.toString(),
    );
    currentRow++;
    ...
    // 4. Items Table Rows
    for (int i = 0; i < proforma.items.length; i++) {
      var item = proforma.items[i];
    ...
    addTotalRow('Subtotal', proforma.subtotal, false);
    if ((proforma.isVatApplicable ?? true) && proforma.taxAmount > 0) {
      addTotalRow('VAT', proforma.taxAmount, false);
    }
    addTotalRow('Total', proforma.total, true);
    currentRow += 2;
